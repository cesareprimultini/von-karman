...
def _solve_beam_state(segments, u_w_all, idx, wave_dir_vec, curr_vec, sign,
                      beam_axis, load_positions, edge_positions, eval_points,
                      midspan, L_beam, rho, Cd, D):
    """Solve the fixed-fixed beam for one wave state (crest or trough).

    Returns dict with M_bme, M_tdp, M_mid, M_max, M_edges.
    """
    loads = []
    for i, seg in enumerate(segments):
        ...
        # Cross-flow drag per segment
        V_par = np.dot(V_total, seg['axis']) * seg['axis']
        V_norm = V_total - V_par
        V_n = np.linalg.norm(V_norm)

        if V_n > 0:
            # Drag force vector (in direction of normal velocity)
            F_vec = 0.5 * rho * Cd * D * V_n**2 * seg['length'] * (V_norm / V_n)
            # Project onto plane perpendicular to equivalent beam axis
            F_along = np.dot(F_vec, beam_axis) * beam_axis
            F_perp = F_vec - F_along
            F_mag = np.linalg.norm(F_perp)
        else:
            F_mag = 0.0
...
def compute_fls_proxy_ff(segments_path, velocities_path, output_path, cps_heading,
    ...
    for idx in range(n_rows):
        ...
        # Solve beam for crest (+u_w) and trough (-u_w)
        common_args = dict(
        ...
        crest = _solve_beam_state(sign=+1.0, **common_args)
        trough = _solve_beam_state(sign=-1.0, **common_args)

        # M -> curvature -> strain at key locations
        r_outer = D / 2.0
        ...
        delta_strain_max = abs(strain_crest_max - strain_trough_max)
        ...
        delta_strain_bme = abs(strain_crest_bme - strain_trough_bme)
        ...
        delta_strain_tdp = abs(strain_crest_tdp - strain_trough_tdp)

        # Damage calculation (range logic: one count per wave cycle)
